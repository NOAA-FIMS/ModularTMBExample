...
sim.data<-replicate(length(ages), 0.0)

for(i in 1:length(ages)){
  sim.data[i] = (l_inf * (1.0 - exp(-k * (ages[i] - a_min))))* runif(1,.90,1.5)
}
...
obj <- MakeADFun(DatList, ParList, DLL="ModularTMBExample")
...
#GC model and try again
rm(g, vonB, Dat); invisible(gc())
#get the Rcpp module
g<-Rcpp::Module(module = "growth",PACKAGE = "ModularTMBExample")
